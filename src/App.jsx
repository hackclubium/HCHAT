...
function Sidebar({ channels, channelId, dms, dmId, unread, isStaff, onChannel, onDm, onAdmin }) {
  return (
...
export default function App() {
  ...
  async function sendDm(event) { event.preventDefault(); const body = dmMessage.trim(); if (!body || !dmId) return; setDmMessage(''); const { error } = await supabase.from('dm_messages').insert({ conversation_id: dmId, body }); if (error) { setDmMessage(body); setStatus(error.message); } }
